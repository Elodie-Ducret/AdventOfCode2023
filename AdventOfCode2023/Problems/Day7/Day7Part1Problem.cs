...
public class Day7Part1Problem(string name, string path) : Problem<List<Hand>, int>(name, path)
{
    public static void Run()
    {
        var day1Problem = new Day7Part1Problem("Day 7 Part 1", "Day7/Day7.txt");
        var response = day1Problem.SolveProblem();
    }

    public static void RunTest()
    {
        var day1Problem = new Day7Part1Problem("Day 7 Part 1 Test", "Day7/Day7_Part1_Test.txt");
        var response = day1Problem.SolveProblem();
    }
    ...
    {
        var handsByType = input.GroupBy(x => x.GetBestType()).OrderByDescending(c => c.Key).ToList();

        var sum = 0;
        var counter = 1;
        foreach (var group in handsByType)
        {
            var handsOrdered = group.OrderBy(x => x);
